...
class CompteSecretaires
{
    ...
    {
        $req = "UPDATE `secretaires` SET  `prenom` = '" . ucfirst(strtolower($_POST['prenom'])) . "', `nom` = '" . ucfirst(strtolower($_POST['nom'])) . "', `telephone` = '" . $_POST['telephone'] . "', `date_de_naissance` = '" . $_POST['date_de_naissance'] . "', `genre` = '" . $_POST['genre'] . "', `ville` = '" . ucfirst(strtolower($_POST['ville'])) . "', `adresse` = '" . $_POST['adresse'] . "' WHERE `secretaires`.`id` = '" . $_SESSION['compte']['id'] . "' AND `secretaires`.`mote_de_pass` = '" . $_SESSION['compte']['mote_de_pass'] . "'";
        $query = mysqli_query(CONNECT, $req);
        return $query == 1 ? 1 : 0;
    }
    public static function mise_a_jour_image_de_profil()
    {
        // supprimer luncien profile
        try {
            $req_0 = "select image_de_profil from secretaires where id='" . $_SESSION['compte']['id'] . "'";
            $query = mysqli_query(CONNECT, $req_0);
            $name_prfl = mysqli_fetch_column($query);
            if ($name_prfl) :
                $path_profile = $_SERVER['DOCUMENT_ROOT'] . '/urbaine/data/profiles/' . $name_prfl;
        ...
        $req = "UPDATE `secretaires` SET `image_de_profil` = '" . $name_image_de_profil . "'  WHERE `secretaires`.`id` = '" . $_SESSION['compte']['id'] . "' AND `secretaires`.`mote_de_pass` = '" . $_SESSION['compte']['mote_de_pass'] . "'";
        $query = mysqli_query(CONNECT, $req);
        return $query;
    }
    public static function mise_a_jour_email()
    {
        ...
        $req1 = "SELECT * FROM secretaires where id='" . $_SESSION['compte']['id'] . "' and email='" . $_SESSION['compte']['email'] . "' and mote_de_pass='" . md5($_POST['password']) . "'";
        //return $req1;
        ...
        $req3 = "update secretaires set email='" . $_POST['email'] . "' WHERE id='" . $_SESSION['compte']['id'] . "' and email='" . $_SESSION['compte']['email'] . "' and mote_de_pass='" . md5($_POST['password']) . "'";
...
?>
    ...
    public static function mise_a_jour_status()
    {
        ...
        $req = "UPDATE `secretaires` SET `derniere_vu` = CURRENT_TIMESTAMP WHERE `id` = '" . $_SESSION['compte']['id'] . "'";
        $query = mysqli_query(CONNECT, $req);
        return $query;
    }
...
switch (@$_GET['do']) {
    case 'mise_a_jour_info':
        ...
        if ($result) {
            $req = "SELECT * FROM secretaires WHERE email='" . $_SESSION['compte']['email'] . "' and mote_de_pass='" . $_SESSION['compte']['mote_de_pass'] . "'";
    ...
    case 'mise_a_jour_email':
        ...
        if ($result) {
            $req = "SELECT * FROM secretaires WHERE id='" . $_SESSION['compte']['id'] . "' and mote_de_pass='" . $_SESSION['compte']['mote_de_pass'] . "'";
            $_SESSION['compte'] = mysqli_fetch_assoc(mysqli_query(CONNECT, $req));
        }
        echo $result;
        exit();
    case 'mise_a_jour_mote_de_pass':
        ...
        if ($result) {
            $req = "SELECT * FROM secretaires WHERE id='" . $_SESSION['compte']['id'] . "' and email='" . $_SESSION['compte']['email'] . "'";
            $_SESSION['compte'] = mysqli_fetch_assoc(mysqli_query(CONNECT, $req));
        }
        echo $result;
        exit();
    ...
    */
    ...
    ?>
        <div class="mb-3 pt-4">
            <label class="form-label">Envoyeur</label>
            <div class="form-control">
                <?= date('h:i A d/m/y', strtotime($demande['envoyeur'])) ?>
            </div>
        </div>
        <div class="mb-3">
            <label class="form-label">Titre</label>
